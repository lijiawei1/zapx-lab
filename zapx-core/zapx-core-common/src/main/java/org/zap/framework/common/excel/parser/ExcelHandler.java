...
public class ExcelHandler {
	 ...
	 * @param data
	 */
	public void appendRange(ExcelCell topLeftCell, List<Object[]> data) {
		appendRange(getSheet(topLeftCell), topLeftCell.getRowIndex(), topLeftCell.getColumnIndex(), data);
	}

	private void appendRange(Sheet sheet, int rowFrom, int colFrom, List<Object[]> data) {
		int rowIndex = rowFrom;
		for (Object[] dataRow : data) {
			Row row = sheet.getRow(rowIndex);
			int columnIndex = colFrom;
	 ...
	 * @param value
	 */
	public void appendCell(ExcelCell excelCell, Object value) {
		Row row = getSheet(excelCell).getRow(excelCell.getRowIndex());
		Cell cell = row.getCell(excelCell.getColumnIndex());
